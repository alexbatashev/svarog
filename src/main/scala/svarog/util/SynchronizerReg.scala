...
/** These wrap behavioral shift and next registers into specific modules to
  * allow for backend flows to replace or constrain them properly when used for
  * CDC synchronization, rather than buffering.
  *
  * These are built up of *ResetSynchronizerPrimitiveShiftReg, intended to be
  * replaced by the integrator's metastable flops chains or replaced at this
  * level if they have a multi-bit wide synchronizer primitive. The different
  * types vary in their reset behavior: NonSyncResetSynchronizerShiftReg --
  * Register array which does not have a reset pin
  * AsyncResetSynchronizerShiftReg -- Asynchronously reset register array,
  * constructed from W instantiations of D deep 1-bit-wide shift registers.
  * SyncResetSynchronizerShiftReg -- Synchronously reset register array,
  * constructed similarly to AsyncResetSynchronizerShiftReg
  *
  * [Inferred]ResetSynchronizerShiftReg -- TBD reset type by chisel3 reset
  * inference.
  *
  * ClockCrossingReg -- Not made up of SynchronizerPrimitiveShiftReg. This is
  * for single-deep flops which cross Clock Domains.
  */
...
class SyncResetSynchronizerShiftReg(w: Int = 1, sync: Int, init: Int)
    extends AbstractPipelineReg(w) {
  require(sync > 1, s"Sync must be greater than 1, not ${sync}.")
  override def desiredName =
...
class ResetSynchronizerShiftReg(w: Int = 1, sync: Int, init: Int)
    extends AbstractPipelineReg(w) {
  require(sync > 1, s"Sync must be greater than 1, not ${sync}.")
  override def desiredName =
...
class SynchronizerShiftReg(w: Int = 1, sync: Int = 3)
    extends AbstractPipelineReg(w) {
  require(sync > 1, s"Sync must be greater than 1, not ${sync}.")
...
class ClockCrossingReg(w: Int = 1, doInit: Boolean) extends Module {
  ...
  val cdc_reg =
    if (doInit) RegEnable(io.d, 0.U(w.W), io.en) else RegEnable(io.d, io.en)
  io.q := cdc_reg
}
