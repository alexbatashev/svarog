...
class AsyncQueueSource[T <: Data](
  ...
  val mem = Reg(
    Vec(params.depth, gen)
  ) // This does NOT need to be reset at all.
  ...
  val ready =
    sink_ready && widx =/= (ridx ^ (params.depth | params.depth >> 1).U)
...
class AsyncQueue[T <: Data](
  ...
  val source = withClockAndReset(io.enq_clock, io.enq_reset) {
    Module(new AsyncQueueSource(gen, params))
  }
  val sink = withClockAndReset(io.deq_clock, io.deq_reset) {
    Module(new AsyncQueueSink(gen, params))
  }
