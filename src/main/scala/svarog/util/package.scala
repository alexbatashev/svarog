...
package object util {
  ...
  implicit class VecToAugmentedVec[T <: Data](private val x: Vec[T])
      extends AnyVal {

    /** Like Vec.apply(idx), but tolerates indices of mismatched width */
    def extract(idx: UInt): T = x(
      (idx | 0.U(log2Ceil(x.size).W)).extract(log2Ceil(x.size) - 1, 0)
    )
  }
  ...
  implicit class DataToAugmentedData[T <: Data](private val x: T)
      extends AnyVal {
  ...
  implicit class SeqMemToAugmentedSeqMem[T <: Data](
      private val x: SyncReadMem[T]
  ) extends AnyVal {
    def readAndHold(addr: UInt, enable: Bool): T =
      x.read(addr, enable) holdUnless RegNext(enable)
  }
  ...
  implicit class StringToAugmentedString(private val x: String) extends AnyVal {
    ...
    def underscore: String =
      x.tail.foldLeft(x.headOption.map(c => s"${c.toLower}") getOrElse "") {
  ...
  implicit class UIntToAugmentedUInt(private val x: UInt) extends AnyVal {
    ...
    def rotateRight(n: UInt): UInt = {
      if (x.getWidth <= 1) {
        ...
        (0 until log2Ceil(x.getWidth)).foldLeft(x)((r, i) =>
          Mux(amt(i), r.rotateRight(1 << i), r)
        )
      }
    }

    def rotateLeft(n: Int): UInt = if (n == 0) x
    ...
    def addWrap(y: UInt, n: Int): UInt = {
      val z = x +& y
      if (isPow2(n)) z(n.log2 - 1, 0)
      else Mux(z >= n.U, z - n.U, z)(log2Ceil(n) - 1, 0)
    }
    ...
    def subWrap(y: UInt, n: Int): UInt = {
      val z = x -& y
      if (isPow2(n)) z(n.log2 - 1, 0)
      else Mux(z(z.getWidth - 1), z + n.U, z)(log2Ceil(n) - 1, 0)
    }
  ...
  implicit class OptionUIntToAugmentedOptionUInt(private val x: Option[UInt])
      extends AnyVal {
    def ##(y: UInt): UInt = x.map(_ ## y).getOrElse(y)
    def ##(y: Option[UInt]): Option[UInt] = x.map(_ ## y)
  }

  implicit class BooleanToAugmentedBoolean(private val x: Boolean)
      extends AnyVal {
  ...
  def UIntToOH1(x: UInt, width: Int): UInt =
    ~((-1).S(width.W).asUInt << x)(width - 1, 0)
  ...
  /** Similar to Seq.groupBy except this returns a Seq instead of a Map Useful
    * for deterministic code generation
    */
  def groupByIntoSeq[A, K](
      xs: Seq[A]
  )(f: A => K): immutable.Seq[(K, immutable.Seq[A])] = {
  ...
  def heterogeneousOrGlobalSetting[T](in: Seq[T], n: Int): Seq[T] =
    in.size match {
      case 1           => List.fill(n)(in.head)
      case x if x == n => in
      case _ =>
        throw new Exception(
          s"must provide exactly 1 or $n of some field, but got:\n$in"
        )
    }
