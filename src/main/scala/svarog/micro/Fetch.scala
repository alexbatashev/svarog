...
import svarog.bpu.StaticBranchPredictor

class FetchIO(xlen: Int) extends Bundle {
  val instOut = Decoupled(new DecoderInput(xlen))
...
class Fetch(xlen: Int, resetVector: BigInt = 0) extends Module {
  ...
  val nextPC = RegInit(resetVector.U(xlen.W))

  val predictor = Module(new StaticBranchPredictor(xlen))

  when(!io.halt && instOut.ready) {
  ...
  // private val predictor = Module(new StaticBranchPredictor(xlen))

  // val isBranch = false.B

  // val pc_plus_4 = pc_reg + 4.U

  // val predictedNext = WireInit(resetVector.U(xlen.W))
  ...
  //   reqPending := true.B
  //   respPC := pc_reg
  //   respPredTaken := predictor.io.predicted.taken
  ...
  // io.inst_out.bits.data.pc := respPC
  // io.inst_out.bits.data.word := respData
  // io.inst_out.bits.prediction.taken := respPredTaken
  // io.inst_out.bits.prediction.target := nextPC
