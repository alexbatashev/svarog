...
class FetchSpec extends AnyFlatSpec with Matchers with ChiselSim {
  ...
  it should "issue sequential fetches and output pc/instruction pairs" in {
    ...
    simulate(new Fetch(xlen, resetVector = 0)) { dut =>
      ...
      dut.io.predictorUpdate.valid.poke(false.B)
      dut.io.predictorUpdate.bits.pc.poke(0.U)
      dut.io.predictorUpdate.bits.targetPC.poke(0.U)
      dut.io.predictorUpdate.bits.taken.poke(false.B)
      dut.io.predictorUpdate.bits.isBranch.poke(false.B)
      dut.io.predictorUpdate.bits.isUncond.poke(false.B)
