...
import svarog.decoder.{ImmFormat, ImmGen}
...
class ImmGenSpec extends AnyFlatSpec with ChiselSim with Matchers {
  ...
  it should "generate I-type immediates correctly" in {
    simulate(new ImmGen(xlen)) { dut =>
      ...
      testCase(dut, svarog.decoder.ImmFormat.I, 0xfff30293L, -1)
      // Corresponds to: lw x5, 12(x6)
      testCase(dut, svarog.decoder.ImmFormat.I, 0x00c32283L, 12)
    }
  }
  ...
  it should "generate S-type immediates correctly" in {
    simulate(new ImmGen(xlen)) { dut =>
      ...
      testCase(dut, svarog.decoder.ImmFormat.S, 0xfe532e23L, -4)
      // Corresponds to: sh x5, 20(x6)
      testCase(dut, svarog.decoder.ImmFormat.S, 0x01431a23L, 20)
    }
  }
  ...
  it should "generate B-type immediates correctly" in {
    simulate(new ImmGen(xlen)) { dut =>
      ...
      testCase(dut, svarog.decoder.ImmFormat.B, 0xfe629ce3L, -8)
      // Corresponds to: beq x5, x6, 16
      testCase(dut, svarog.decoder.ImmFormat.B, 0x00628863L, 16)
    }
  }
  ...
  it should "generate U-type immediates correctly" in {
    simulate(new ImmGen(xlen)) { dut =>
      ...
      testCase(dut, svarog.decoder.ImmFormat.U, 0xabcde2b7L, 0xabcde000)
      // Corresponds to: auipc x5, 0x12345
      testCase(dut, svarog.decoder.ImmFormat.U, 0x12345297L, 0x12345000)
    }
  }
  ...
  it should "generate J-type immediates correctly" in {
    simulate(new ImmGen(xlen)) { dut =>
      ...
      testCase(dut, svarog.decoder.ImmFormat.J, 0xff1ff2efL, -16)
      // Corresponds to: jal x5, 20
      testCase(dut, svarog.decoder.ImmFormat.J, 0x014002efL, 20)
    }
  }
}
