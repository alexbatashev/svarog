...
class CSRExSpec extends AnyFlatSpec with Matchers with ChiselSim {
  ...
  it should "execute CSRRW (read-write) with register" in {
    simulate(new CSREx(xlen)) { dut =>
      ...
      dut.io.csrWrite.valid.expect(true.B)
      dut.io.csrWrite.bits.expect(0x12345678L.U)

      println(
        s"CSRRW: result=${dut.io.result.bits.peek().litValue}, write=${dut.io.csrWrite.bits.peek().litValue}"
      )
    }
  }
  ...
  it should "execute CSRRWI (read-write-immediate)" in {
    simulate(new CSREx(xlen)) { dut =>
      ...
      dut.io.result.bits.expect(0.U)
      dut.io.csrWrite.valid.expect(true.B)
      dut.io.csrWrite.bits.expect(5.U)

      println(
        s"CSRRWI: result=${dut.io.result.bits.peek().litValue}, write=${dut.io.csrWrite.bits.peek().litValue}"
      )
    }
  }
  ...
  it should "execute CSRRS (read-set) with register" in {
    simulate(new CSREx(xlen)) { dut =>
      ...
      dut.io.csr.data.poke(0x40000100L.U)
      ...
      dut.io.csrWrite.valid.expect(true.B)
      dut.io.csrWrite.bits.expect(0x40001100L.U)

      println(
        f"CSRRS: result=0x${dut.io.result.bits.peek().litValue}%x, write=0x${dut.io.csrWrite.bits.peek().litValue}%x"
      )
    }
  }
  ...
  it should "not write CSR for CSRRS when rs1=x0" in {
    simulate(new CSREx(xlen)) { dut =>
      ...
      dut.io.result.valid.expect(true.B)
      dut.io.result.bits.expect(0x40000100L.U)
      dut.io.csrWrite.valid.expect(false.B) // No write when rs1=0

      println(f"CSRRS (x0): result=0x${dut.io.result.bits
          .peek()
          .litValue}%x, write_en=${dut.io.csrWrite.valid.peek().litToBoolean}")
    }
  }
  ...
  it should "execute CSRRC (read-clear) with register" in {
    simulate(new CSREx(xlen)) { dut =>
      ...
      dut.io.csr.data.poke(0x40001100L.U)
      dut.io.rs1Value.poke(0x00001000L.U)
      ...
      dut.io.csrWrite.valid.expect(true.B)
      dut.io.csrWrite.bits.expect(0x40000100L.U)

      println(
        f"CSRRC: result=0x${dut.io.result.bits.peek().litValue}%x, write=0x${dut.io.csrWrite.bits.peek().litValue}%x"
      )
    }
  }
  ...
  it should "execute CSRRCI (read-clear-immediate)" in {
    simulate(new CSREx(xlen)) { dut =>
      ...
      dut.io.csr.data.poke(0x40001110L.U)
      ...
      println(f"CSRRCI: result=0x${dut.io.result.bits
          .peek()
          .litValue}%x, write=0x${dut.io.csrWrite.bits.peek().litValue}%x")
    }
  }
  ...
  it should "not output when uop is invalid" in {
    simulate(new CSREx(xlen)) { dut =>
      ...
      dut.io.csr.data.poke(0x12345678L.U)

      dut.clock.step(1)

      dut.io.result.valid.expect(false.B)
      dut.io.csrWrite.valid.expect(false.B)
